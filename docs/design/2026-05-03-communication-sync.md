...
| 定数名 | 値 | ボディ | 備考 |
|---|---|---|---|
| `CONNECT` | `0x01` | UTF-8文字列（playerName） | 接続通知。IDはサーバーが採番、名前はクライアントが指定 |
...
| `ROUND_END` | `0x14` | `winner_id(1)` + `correct_index(1)` | 全員 | ラウンド終了 |
| `SCORE` | `0x15` | `num_players(1)` + `[player_id(1) \| score(2)] × n` | 全員 | ROUND_END直後に送信 |
